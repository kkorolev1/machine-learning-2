...
class LogUniformDistribution(UniformDistribution):
    '''
    ...
    def __init__(self, low: float, high: float):
        ...
        super().__init__(np.log(low), np.log(high))

    def sample(self, size: int) -> np.array:
        return np.exp(super().sample(size))

    def scale(self, sample: np.array) -> np.array:
        return super().scale(sample)
...
class IntUniformDistribution(NumericalDistribution):
    ...
    def sample(self, size: int) -> np.array:
        return np.random.randint(low=self.low, high=self.high + 1, size=size)

    def scale(self, sample: np.array) -> np.array:
        return (sample - sample.min()) / (sample.max() - sample.min())


class IntLogUniformDistribution(UniformDistribution):
    '''
    ...
    def sample(self, size: int) -> np.array:
        return np.exp(super().sample(size)).astype(int)
